...
export function EduPiTeachingWorkspace({ data, context, query, selectedObjectId, onObject, onTask, onNavigate, onStartAgent, onCalendarSelection }: { data: EducationContract; context: TeacherContextSnapshot | null; query: string; selectedObjectId: string | null; onObject: (id: string) => void; onTask: (task: TeacherTask) => void; onNavigate: (view: "calendar" | "tasks" | "memory", objectId?: string) => void; onStartAgent: (prompt: string, mode?: "insert" | "replace") => void; onCalendarSelection: (selection: CalendarItemSelection) => void }) {
  ...
  if (section === "tasks") return <main className="edupi-module-workspace edupi-teaching-workspace">{header}<section className="edupi-database"><div className="edupi-database__head edupi-teaching-task-grid"><span>任务</span><span>主题</span><span>截止</span><span>状态</span></div>{tasks.map((task) => <button type="button" className="edupi-database-button-row edupi-teaching-task-grid" key={taskKey(task)} onClick={() => onTask(task)}><strong>{taskDisplayTitle(task)}</strong><span>{task.topic || task.sourceEventName || "教学准备"}</span><span>{task.dueDate || "待确认"}</span><span>{taskStatusLabel(task)}</span></button>)}</section></main>;
  ...
  return <main className="edupi-module-workspace edupi-teaching-workspace">{header}<section className="edupi-teaching-next-line"><div><span>下一节课</span><strong>{nextSubject}</strong><p>{nextSlot ? `周${String(nextSlot.day_of_week)} · 第 ${String(nextSlot.period)} 节${nextSlot.class_name ? ` · ${String(nextSlot.class_name)}` : ""}` : "课表待导入"}</p></div><button type="button" onClick={() => onObject("teaching:schedule")}>课程表</button></section><section className="edupi-teaching-home-section"><header><h2>本周课程</h2><button type="button" onClick={() => onObject("teaching:schedule")}>完整课程表</button></header><EduPiTimetableGrid slots={slots} onSelect={selectCourse} compact /></section><div className="edupi-teaching-home-columns"><section><header><h2>当前教学重点</h2><button type="button" onClick={() => onObject("teaching:knowledge")}>全部</button></header>{knowledge.slice(0, 4).map((item) => <details key={item.id}><summary><strong>{item.topic}</strong><span>{item.commonErrors[0]?.description || "暂无共性问题"}</span></summary><p>{item.strugglingStudents.length ? `需关注：${item.strugglingStudents.join("、")}` : "暂无需特别关注学生"}</p></details>)}</section><section><header><h2>备课任务</h2><button type="button" onClick={() => onObject("teaching:tasks")}>全部</button></header>{tasks.slice(0, 5).map((task) => <button type="button" key={taskKey(task)} onClick={() => onTask(task)}><strong>{taskDisplayTitle(task)}</strong><span>{task.dueDate || taskStatusLabel(task)}</span></button>)}</section></div></main>;
}
