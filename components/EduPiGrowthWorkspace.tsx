...
export function EduPiGrowthWorkspace({ data, teachingSkills, query, selectedObjectId, onOpenFile, onTask }: { data: EducationContract; teachingSkills: EduPiTeachingSkillLifecycle; query: string; selectedObjectId: string | null; onOpenFile: (path: string) => void; onTask: (task: TeacherTask) => void }) {
  ...
  return <main className="edupi-module-workspace edupi-database-workspace">
    <header className="edupi-module-heading"><div><span>成长 / {category === "teacher" ? "教师专业成长" : "EduPi 能力成长"}</span><h1>{category === "teacher" ? "教师专业成长" : "EduPi 能力成长"}</h1><p>{category === "teacher" ? growthSource.present ? "数据已连接 · 来自已确认工作产物" : "教师成长数据尚未接入" : teachingSkills.status === "unavailable" ? "教学能力生命周期尚未接入" : "教学能力生命周期已连接"}</p></div></header>
    {category === "teacher" ? <section className="edupi-database"><div className="edupi-database__head edupi-growth-db-grid"><span>类型</span><span>内容</span><span>来源</span><span>日期</span><span>动作</span></div>{documents.map((item) => <div className="edupi-database-static-row edupi-growth-db-grid" key={item.id}><span>{item.kind === "weekly" ? "周复盘" : "专业反思"}</span><strong>{item.title}</strong><span>工作沉淀</span><time>{shortDate(item.date)}</time><button type="button" onClick={() => onOpenFile(workspaceFile(item.path))}>打开</button></div>)}{artifacts.map(({ task, artifact }) => <div className="edupi-database-static-row edupi-growth-db-grid" key={artifact.id}><span>确认成果</span><strong>{artifact.title}</strong><span>{taskDisplayTitle(task)}</span><time>{task.reviewedAt ? shortDate(task.reviewedAt) : "—"}</time><button type="button" onClick={() => onTask(task)}>查看</button></div>)}{documents.length + artifacts.length === 0 ? <div className="edupi-database__empty">{growthSource.present ? "数据已连接，暂无专业成长记录" : "教师成长数据尚未接入"}</div> : null}</section> : <section className="edupi-database"><div className="edupi-database__head edupi-edupi-growth-db-grid"><span>教学能力</span><span>状态</span><span>试用</span><span>依据</span><span>最近更新</span></div>{skills.map((item) => <details className="edupi-database-row" key={item.skillId}><summary className="edupi-edupi-growth-db-grid"><strong>{item.title}</strong><span>{SKILL_STATE_LABELS[item.lifecycleState] || item.lifecycleState}</span><span>{item.trialCount}</span><span>{item.evidenceIds.length}</span><time>{shortDate(item.updatedAt)}</time></summary><div className="edupi-database-row__detail"><div><span>能力 ID</span><strong>{item.skillId}</strong></div><div><span>复用状态</span><strong>{item.canReuse ? "可复用" : "继续验证"}</strong></div><div><span>证据</span><strong>{item.evidenceIds.length ? `${item.evidenceIds.length} 条已保留` : "尚无证据"}</strong></div></div></details>)}{skills.length === 0 ? <div className="edupi-database__empty">{teachingSkills.status === "unavailable" ? "教学能力生命周期尚未接入" : "数据已连接，暂无 EduPi 能力成长记录"}</div> : null}</section>}
  </main>;
}
