...
export function EduPiObjectSider({ view, data, context, query, selectedTaskKey, onTask, onReviewTarget, selectedCalendarSourceId, onCalendarItem, onUpload, onCollapse }: Props) {
  ...
  const upcoming = tasks.filter((task) => task.boardStage !== "done" && (task.status === "planned" || task.status === "hold") && !isTaskActionable(task));
  ...
  return (
    <EduPiContentSider width={244} ariaLabel={`${viewTitles[view]}对象列表`} header={<div className="edupi-object-sider__header"><div><span>当前模块</span><strong>{viewTitles[view]}</strong></div><button type="button" onClick={onCollapse} aria-label="收起列表">‹</button></div>}>
      ...
      {view === "tasks" ? <><section className="edupi-object-group"><GroupTitle count={pending.length}>现在需要处理</GroupTitle>{taskRows(pending, "brief")}</section><section className="edupi-object-group"><GroupTitle count={upcoming.length}>以后再处理</GroupTitle>{taskRows(upcoming.slice(0, 10), "brief")}</section><section className="edupi-object-group"><GroupTitle count={reviewed.length}>已处理</GroupTitle>{taskRows(reviewed, "brief")}</section></> : null}
      ...
      {view === "growth" ? <><section className="edupi-object-group"><GroupTitle count={documents.length}>沉淀记录</GroupTitle>{documents.slice(0, 6).map((document) => <div className="edupi-object-fact" key={document.id}><strong>{document.title}</strong><span>{objectStudentDateLabel(document.date) || document.path}</span></div>)}</section><section className="edupi-object-group"><GroupTitle count={themes.length}>学习候选</GroupTitle>{themes.map((theme) => <div className="edupi-object-fact" key={theme.topic}><strong>{theme.topic}</strong><span>出现 {theme.occurrences} 次 · {theme.skillCandidate ? "待验证" : "观察中"}</span></div>)}</section></> : null}
