  ...
  return <section className={`edupi-timetable-grid${compact ? " is-compact" : ""}`} aria-label="教师课程表">
    ...
    {PERIODS.flatMap((period) => {
      ...
      for (let day = 1; day <= 5; day += 1) {
        ...
        row.push(<div className={`edupi-timetable-grid__cell${period === 6 ? " is-afternoon-start" : ""}`} key={`${day}:${period}`}>{cellSlots.map((slot, index) => {
          const sourceId = text(slot.slot_id ?? slot.id) || `timetable:${day}:${period}:${index}`;
          ...
          return <button type="button" key={sourceId} onClick={() => onSelect?.({ kind: "timetable", sourceId, date: null, title: subject, detail: `周${day} · 第 ${period} 节`, sourceLabel: recognized ? "材料识别" : "课程表", statusLabel: recognized ? "待确认" : "已确认" })}><strong>{subject}</strong>{className ? <span>{className}</span> : null}</button>;
        })}</div>);
      }
      return row;
    })}
