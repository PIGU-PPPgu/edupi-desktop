...
export function EduPiMaterialsWorkspace({ data, query, selectedObjectId, stagedMaterials, stagingBusy, stagingMessage, onTask, onUpload, onIntakeMaterial, onRemoveStagedMaterial, onStartAgent, onDeleteEntity }: { data: EducationContract; query: string; selectedObjectId: string | null; stagedMaterials: MaterialStagingDescriptor[]; stagingBusy: boolean; stagingMessage: string | null; onTask: (task: TeacherTask) => void; onUpload: () => void; onIntakeMaterial: (item: MaterialStagingDescriptor) => Promise<unknown>; onRemoveStagedMaterial: (item: MaterialStagingDescriptor) => Promise<void>; onStartAgent: (prompt: string, mode?: "insert" | "replace") => void; onDeleteEntity: (kind: EducationEntityDeleteKind, id: string, label: string) => Promise<boolean> }) {
  ...
  const deleteSelected = async () => {
    ...
    setDeleteBusy(true);
    try {
      const deleted = selected.task?.id
  ...
  return <main className="edupi-module-workspace edupi-database-workspace">
    ...
    {selected ? <aside className="edupi-material-drawer" aria-label={`${selected.title}材料详情`}><header><div><span>{selected.type}</span><h2>{selected.title}</h2></div><button type="button" onClick={() => setSelected(null)} aria-label="关闭材料详情">×</button></header><dl><div><dt>状态</dt><dd>{selected.status}</dd></div><div><dt>来源</dt><dd>{selected.source}</dd></div><div><dt>学科 / 班级</dt><dd>{selected.subject}</dd></div><div><dt>日期</dt><dd>{shortDate(selected.date)}</dd></div><div><dt>说明</dt><dd>{selected.summary}</dd></div></dl><footer>{selected.task ? <button type="button" onClick={() => onTask(selected.task!)}>打开关联任务</button> : null}{data.capabilities.entityDelete.enabled && data.capabilities.entityDelete.targetKinds.includes(selected.task ? "task" : "material") && (!selected.task || selected.task.id) ? <button type="button" className="is-delete" disabled={deleteBusy} onClick={() => void deleteSelected()}>{deleteBusy ? "删除中…" : "删除材料"}</button> : null}<button type="button" className="is-primary" onClick={openMaterialAgent}>补充 / 修订</button></footer></aside> : null}
  </main>;
}
