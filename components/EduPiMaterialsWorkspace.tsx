...
export function EduPiMaterialsWorkspace({ data, query, selectedObjectId, stagedMaterials, stagingBusy, stagingMessage, onTask, onUpload, onIntakeMaterial, onRemoveStagedMaterial, onStartAgent }: { data: EducationContract; query: string; selectedObjectId: string | null; stagedMaterials: MaterialStagingDescriptor[]; stagingBusy: boolean; stagingMessage: string | null; onTask: (task: TeacherTask) => void; onUpload: () => void; onIntakeMaterial: (item: MaterialStagingDescriptor) => Promise<unknown>; onRemoveStagedMaterial: (item: MaterialStagingDescriptor) => Promise<void>; onStartAgent: (prompt: string) => void }) {
  ...
  const rows = useMemo<MaterialRow[]>(() => {
    ...
    return [...taskRows, ...intakeRows].filter((item) => category === "all" || item.category === category).filter((item) => !query || `${item.title} ${item.type} ${item.subject} ${item.summary}`.toLocaleLowerCase().includes(query.toLocaleLowerCase())).sort((left, right) => String(right.date || "").localeCompare(String(left.date || "")));
  ...
  return <main className="edupi-module-workspace edupi-database-workspace">
    ...
    {selected ? <aside className="edupi-material-drawer" aria-label={`${selected.title}材料详情`}><header><div><span>{selected.type}</span><h2>{selected.title}</h2></div><button type="button" onClick={() => setSelected(null)} aria-label="关闭材料详情">×</button></header><dl><div><dt>状态</dt><dd>{selected.status}</dd></div><div><dt>来源</dt><dd>{selected.source}</dd></div><div><dt>学科 / 班级</dt><dd>{selected.subject}</dd></div><div><dt>日期</dt><dd>{shortDate(selected.date)}</dd></div><div><dt>说明</dt><dd>{selected.summary}</dd></div></dl><footer>{selected.task ? <button type="button" onClick={() => onTask(selected.task!)}>打开关联任务</button> : null}<button type="button" className="is-primary" onClick={() => onStartAgent(`请补充或修订这份材料的信息：${selected.title}\n当前说明：${selected.summary}\n请保留原始来源，先给出修改候选，待我确认后写回。`)}>补充 / 修订</button></footer></aside> : null}
  </main>;
}
