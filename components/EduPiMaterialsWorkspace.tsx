...
export function EduPiMaterialsWorkspace({ data, query, selectedObjectId, stagedMaterials, stagingBusy, stagingMessage, onTask, onUpload, onIntakeMaterial, onRemoveStagedMaterial, onOpenFile, onStartAgent, onDeleteEntity }: { data: EducationContract; query: string; selectedObjectId: string | null; stagedMaterials: MaterialStagingDescriptor[]; stagingBusy: boolean; stagingMessage: string | null; onTask: (task: TeacherTask) => void; onUpload: () => void; onIntakeMaterial: (item: MaterialStagingDescriptor) => Promise<unknown>; onRemoveStagedMaterial: (item: MaterialStagingDescriptor) => Promise<void>; onOpenFile: (path: string) => void; onStartAgent: (prompt: string, mode?: "insert" | "replace") => void; onDeleteEntity: (kind: EducationEntityDeleteKind, id: string, label: string) => Promise<boolean> }) {
  ...
  const [generated, setGenerated] = useState<GeneratedArtifact[]>([]);
  const [generatedError, setGeneratedError] = useState(false);
  useEffect(() => {
    let active = true;
    const load = async () => {
      try {
        const response = await fetch("/api/edupi/artifacts", { cache: "no-store" });
        if (!response.ok) throw new Error("unavailable");
        const result = await response.json();
        if (active) { setGenerated(result.artifacts || []); setGeneratedError(false); }
      } catch { if (active) setGeneratedError(true); }
    };
    void load();
  ...
  const rows = useMemo<MaterialRow[]>(() => {
    ...
    const generatedRows: MaterialRow[] = generated.map((item) => ({ id: item.artifact_id, title: item.title, category: materialCategory({ title: item.title }), type: "对话生成", subject: "—", source: "EduPi 对话", date: item.updated_at, status: "已生成", summary: item.title, task: data.tasks.find((task) => task.id === item.task_id) || null, filePath: workspaceFile(data.workspace, item.relative_path), deleteKind: null }));
    for (const item of [...generatedRows, ...documentRows, ...artifactRows, ...taskRows, ...intakeRows]) unique.set(item.filePath || item.id, item);
  ...
  }, [category, data.continuity.documents, data.intakeTargets, data.tasks, data.workspace, generated, query]);
