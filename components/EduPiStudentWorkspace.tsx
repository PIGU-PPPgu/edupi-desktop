...
export function EduPiStudentWorkspace({ mode, data, context, query, selectedStudentId, onStudent, onEducation, onTask, onStartAgent, onDeleteEntity }: Props) {
  ...
  const students = data.students.filter(student => !classFilter || student.class_name === classFilter).filter((student) => !query || JSON.stringify(student).toLocaleLowerCase().includes(query.toLocaleLowerCase())).slice().sort((left, right) => studentRecordName(left).localeCompare(studentRecordName(right), "zh-CN"));
