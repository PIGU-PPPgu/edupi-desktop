...
| [ ] | R13 feat(background): OCR 文档 PPT 长任务交付 | 让现有后台队列实际执行 OCR/文档/PPT 任务，展示进度、取消、失败重试与恢复，并统一登记产物。 | 图片→可编辑文本、材料→教案、教案→PPT 各跑一次；重启恢复；文件可打开且内容可核对；失败不留“已完成”假状态。 | R01、R03 | 部分实现；OCR、Word 与重启恢复完整链路待验收 |
