...
R12/R17 正在按 `2026-09-08-student-identity.md` 实施身份兼容，尚未验收完成。教学页审核入口已按实际对象接通同一 Today 审核，刷新与材料状态同步通过。
...
| 学生双网络 R12/R17 | 部分验收通过 | 同名独立选择、模型按 ID 记录、双节点图、页面转班通过；学生删除接口通过；页面确认框因浏览器工具受阻，大班分页和安装版未验收 |
| 首次对话 | 部分验收通过 | 已有模型配置、无会话时实际发送通过；零 API 新手流程仍待验收 |
...
| 后台 R13 | 部分验收通过 | Word 与中文 OCR 实际生成并登记，立即取消、开发进程重启恢复执行、双任务产物独立归属复验通过；干净安装 OCR、页面与安装版恢复仍待验收 |
| 教学方法 R11 | 部分验收通过 | 实际试用、评分、人工内容核对、测试审核、晋级重载、下一课四份产物采用方法通过；首次错误候选已拒绝并留痕；不代表真实课堂效果，页面与方法更新重备课未验收 |
