...
export async function GET(request:Request) {
  ...
  try {return NextResponse.json(await studentEventRequest({action:"list_events",student_id:query.get("student_id")||undefined,student:query.get("student")||undefined,kind:kind||undefined,offset,query:query.get("query")||undefined,from:query.get("from")||undefined,to:query.get("to")||undefined},request.signal));}
...
export async function POST(request:Request) {
  ...
  try {
    ...
    if (body?.action === "set_topic_alias" && Object.keys(body).every(key => ["action", "alias", "canonical"].includes(key))) return NextResponse.json(await studentEventRequest(body, request.signal));
    ...
    return NextResponse.json(await studentEventRequest(body,request.signal));
