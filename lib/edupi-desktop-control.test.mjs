...
test("desktop control accepts only allowlisted EduPi navigation commands", () => {
  ...
  assert.deepEqual(validateDesktopCommand({ action: "open_document", documentId: "daily:2026-09-08" }), { action: "open_document", documentId: "daily:2026-09-08" });
  assert.throws(() => validateDesktopCommand({ action: "open_document", documentId: "../../secret" }));
