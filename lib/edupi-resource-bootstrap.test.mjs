...
const { prepareEducationResources, allowedEducationExtensions } = await createJiti(import.meta.url).import("./edupi-runtime.ts");
...
test("resource setup preserves teacher skills and aligns extension storage with Core", () => {
  ...
  const keys = ["EDUPI_PROJECT_ROOT", "EDUPI_HOME", "EDUPI_MEMORY_DIR", "EDUPI_OUTPUT_DIR", "EDUPI_LOCK_DIR", "EDUPI_SKILLS_DIR", "EDUPI_DATA_DIR", "EDUPI_CONFIG_DIR"];
...
test("loads only Core-manifest education extensions when an inventory exists", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "edupi-inventory-"));
  try {
    fs.mkdirSync(path.join(root, "contracts"));
    fs.writeFileSync(path.join(root, "contracts/edupi-desktop-component-manifest.json"), JSON.stringify({ modules: [], assets: [{ path: "extensions/memory.ts" }] }));
    assert.deepEqual(allowedEducationExtensions(root, ["memory.ts", "evolution_engine.ts", "auto_extract.ts"]), [path.join(root, "extensions/memory.ts")]);
