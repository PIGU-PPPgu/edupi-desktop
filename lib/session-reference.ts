...
export async function resolveSessionReferences(
  ...
  if (missingLabels.length > 0) {
    try {
      const response = await fetchImpl("/api/sessions", { signal: options.signal });
      if (!response.ok && options.strict) throw new Error("session_reference_unavailable");
      if (response.ok) {
  ...
  const references = await Promise.all(targetIds.map(async (id) => {
    try {
      const response = await fetchImpl(`/api/sessions/${encodeURIComponent(id)}/reference`, { signal: options.signal });
      if (!response.ok && options.strict) throw new Error("session_reference_unavailable");
      if (!response.ok) return [id, ""] as const;
      const data = await response.json() as { reference?: string };
      if (options.strict && (typeof data.reference !== "string" || !data.reference.trim())) {
