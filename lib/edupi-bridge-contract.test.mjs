...
test("mirrors Core C1 identity ordering without mutating the visible snapshot", () => {
  ...
  for (const key of ["observations", "memory_candidates", "memories", "receipts", "review_history", "review_targets"]) reversed[key].reverse();
...
test("pins the Core v1.1 education projection runtime capability manifest", () => {
  ...
  assert.equal(manifest.core_runtime.core_commit, "5538021f171a647d87562d91e5ab953f794e2331");
  assert.equal(manifest.core_runtime.component_manifest_hash, "sha256:649c13fd9d6f0defa56a189176cab0e2a461355131397a4601d260fa1d08d922");
  assert.equal(manifest.contract_identities.length, 1);
  assert.deepEqual(manifest.supported_commands, ["review_observation", "review_memory_candidate", "review_teacher_context", "review_work_candidate", "import_calendar", "import_timetable", "intake_material", "create_task", "move_task_stage"]);
  assert.deepEqual(manifest.contract_identities[0].supported_commands, ["review_observation", "review_memory_candidate", "review_teacher_context", "review_work_candidate", "import_calendar", "import_timetable", "intake_material", "create_task", "move_task_stage"]);
  ...
  assert.equal(Object.keys(manifest.unsupported_command_reasons).length, 10);
});
...
test("mirrors the complete Core command and decision vocabulary", { skip: !coreRoot }, async () => {
  ...
  assert.equal(Object.keys(contract.TARGET_COMMANDS).length, 14);
});
