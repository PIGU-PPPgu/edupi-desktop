...
test("pins the Core v1.1 education projection runtime capability manifest", () => {
  ...
  const schema = JSON.parse(fs.readFileSync(new URL("../contracts/edupi-bridge-v1.1.schema.json", import.meta.url), "utf8"));
  const canonicalize = (value) => Array.isArray(value)
    ? value.map(canonicalize)
    : value && typeof value === "object"
      ? Object.fromEntries(Object.keys(value).sort().map((key) => [key, canonicalize(value[key])]))
      : value;
  const localSchemaHash = `sha256:${crypto.createHash("sha256").update(JSON.stringify(canonicalize(schema))).digest("hex")}`;
  assert.equal(manifest.core_runtime.core_commit, "5f5b98bd372efe90b58dab931f08938a739ed91d");
  assert.equal(manifest.core_runtime.component_manifest_hash, "sha256:be39338497f9bfd0f538a7351424c0d3cd49388e0dd2a4f0d5b16a7a7049062a");
  ...
  assert.equal(localSchemaHash, manifest.contract_identities[0].schema_hash, "Desktop frozen schema content must match the pinned Core identity");
