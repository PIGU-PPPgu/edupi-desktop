...
export type Reminder = { id: string; taskId: string; title: string; kind: "ready" | "failed" | "due"; identity: string; createdAt: string; read: boolean; handled: boolean; snoozedUntil: string | null; notificationAttemptedAt?: string };
type Store = { version: 1; items: Reminder[]; notifications?: Reminder[] };
export async function updateReminderStore(file: string, snapshot: Record<string, ReminderEvent>, action?: { id: string; type: "read" | "handled" | "snooze" | "claim_notifications" }, now = Date.now()): Promise<Store> {
  ...
  try {
    ...
    const notifications = action?.type === "claim_notifications" ? state.items.filter(item => !item.handled && !item.read && !item.snoozedUntil && !item.notificationAttemptedAt) : [];
