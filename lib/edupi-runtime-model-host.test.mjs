...
test("configured model host runs isolated localhost SDK and cancellation reaps its connection", { skip: !process.env.EDUPI_CORE_ROOT }, async () => {
  ...
  const config = { providers: { local: { baseUrl: `http://127.0.0.1:${server.address().port}/v1`, api: "openai-completions", models: [{ id: "local", name: "local", reasoning: false, input: ["text"], cost: { input: 0, output: 0, cacheRead: 0, cacheWrite: 0 }, contextWindow: 32000, maxTokens: 128 }] } } };
  fs.writeFileSync(path.join(agentDir, "models.json"), JSON.stringify(config));
  const host = createRuntimeModelHost({ coreRoot: process.env.EDUPI_CORE_ROOT, projectRoot: root, agentDir, allowLoopback: true });
  try {
