...
test("uses the EduPi Desktop source and release repository", () => {
...
test("keeps cached available releases visible to the startup reminder", () => {
  assert.deepEqual(getAvailableAppUpdates([{
    ...
    name: "EduPi Desktop",
    repository: "PIGU-PPPgu/edupi-desktop",
    repositoryUrl: "https://github.com/PIGU-PPPgu/edupi-desktop",
    currentVersion: "0.3.0",
    latestVersion: "0.3.1",
    releaseUrl: "https://github.com/PIGU-PPPgu/edupi-desktop/releases/tag/v0.3.1",
...
test("returns a newer official GitHub release", async () => {
  ...
  const update = await checkAppUpdate(project, {
    fetcher: async (url) => {
      ...
      return jsonResponse({
        tag_name: "v0.8.6",
        html_url: "https://github.com/PIGU-PPPgu/edupi-desktop/releases/tag/v0.8.6",
        draft: false,
        prerelease: false,
      });
    },
  ...
  assert.match(requestedUrl, /repos\/PIGU-PPPgu\/edupi-desktop\/releases\/latest$/);
  assert.deepEqual(update, {
    project: "edupi-desktop",
    name: "EduPi Desktop",
    currentVersion: "0.8.5",
    latestVersion: "0.8.6",
    releaseUrl: "https://github.com/PIGU-PPPgu/edupi-desktop/releases/tag/v0.8.6",
  });
});
...
test("does not report the installed release or prereleases", async () => {
  const current = await checkAppUpdate(project, {
    fetcher: async () => jsonResponse({
      tag_name: "v0.8.5",
      html_url: "https://github.com/PIGU-PPPgu/edupi-desktop/releases/tag/v0.8.5",
    }),
  });
  ...
  await assert.rejects(
    checkAppUpdate(project, {
      fetcher: async () => jsonResponse({
        ...
        html_url: "https://github.com/PIGU-PPPgu/edupi-desktop/releases/tag/v0.9.0-beta.1",
        prerelease: true,
      }),
    }),
    /non-stable/i,
