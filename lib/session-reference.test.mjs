...
test("strict expansion propagates reference fetch errors and uses the caller signal", async () => {
  const { resolveSessionReferences } = await loadSubject();
  const controller = new AbortController();
  await assert.rejects(resolveSessionReferences("#picked", new Map([["picked", "s-1"]]), async (_url, init) => {
    assert.equal(init.signal, controller.signal);
    throw new Error("offline");
  }, { strict: true, signal: controller.signal }), /offline/);
});

test("strict expansion rejects failed HTTP and malformed reference responses", async () => {
  const { resolveSessionReferences } = await loadSubject();
  for (const response of [{ ok: false }, { ok: true, json: async () => ({}) }]) {
    await assert.rejects(resolveSessionReferences("#picked", new Map([["picked", "s-1"]]),
      async () => response, { strict: true }), /session_reference_unavailable/);
  }
});
