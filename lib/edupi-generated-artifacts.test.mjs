...
const { snapshotGeneratedFiles, generatedArtifactsRequest, recoverSessionArtifacts, resolveArtifactRegistrationFields, completedSessionFiles, bashToolArtifactPaths, nativeToolArtifactPath, writtenToolArtifactPath } = await jiti.import("./edupi-generated-artifacts.ts");
...
test("history recovery restores successful bash writes from approved output roots", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "edupi-bash-artifact-history-"));
  try {
    fs.mkdirSync(path.join(root, ".edupi/output"), { recursive: true });
    fs.writeFileSync(path.join(root, ".edupi/output/bash.md"), "# bash");
    fs.writeFileSync(path.join(root, "unrelated.md"), "ignore");
    const command = "printf '# bash' > .edupi/output/bash.md && printf 'ignore' > unrelated.md";
    assert.deepEqual(bashToolArtifactPaths(command, root), [path.join(root, ".edupi/output/bash.md")]);
    const entries = [
      { message: { role: "assistant", content: [{ type: "toolCall", id: "bash", name: "bash", arguments: { command } }] } },
      { message: { role: "toolResult", toolCallId: "bash", isError: false } },
    ];
    assert.deepEqual(completedSessionFiles(entries, root), [path.join(root, ".edupi/output/bash.md")]);
    assert.deepEqual(completedSessionFiles([{ ...entries[1], message: { ...entries[1].message, isError: true } }].concat(entries.slice(0, 1)), root), []);
  } finally { fs.rmSync(root, { recursive: true, force: true }); }
});

test("bash recovery only accepts output operands, never files merely read by the command", () => {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "edupi-bash-output-operands-"));
  try {
    fs.mkdirSync(path.join(root, ".edupi/output"), { recursive: true });
    const input = path.join(root, ".edupi/output/old.md");
    const output = path.join(root, ".edupi/output/new.pdf");
    fs.writeFileSync(input, "old");
    fs.writeFileSync(output, "new");
    assert.deepEqual(bashToolArtifactPaths(`cat .edupi/output/old.md`, root), []);
    assert.deepEqual(bashToolArtifactPaths(`pandoc .edupi/output/old.md -o .edupi/output/new.pdf`, root), [output]);
    assert.deepEqual(bashToolArtifactPaths(`cat .edupi/output/old.md > .edupi/output/new.pdf`, root), [output]);
    assert.deepEqual(bashToolArtifactPaths(`pandoc .edupi/output/old.md -o /tmp/result.pdf`, root), []);
  } finally { fs.rmSync(root, { recursive: true, force: true }); }
});
