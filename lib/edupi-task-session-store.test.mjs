...
test("task-session bindings use a deterministic Desktop/Pi-agent path per data root", async () => {
  ...
  const dataAlias = join(root, "teacher-a-alias");
  await mkdir(dataRoot);
  await mkdir(otherDataRoot);
  await symlink(dataRoot, dataAlias, "dir");
  const previousAgentDir = process.env.PI_CODING_AGENT_DIR;
  process.env.PI_CODING_AGENT_DIR = agentDir;
  try {
    const file = taskSessionFile(dataRoot);
    const rootHash = createHash("sha256").update(realpathSync(dataRoot)).digest("hex");
    ...
    assert.equal(file, taskSessionFile(dataAlias));
