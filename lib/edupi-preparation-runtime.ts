...
type PreparationStatus = { state: "idle" | "running" | "ready" | "error"; updatedAt: string | null; prepared: number; error: string | null };
...
export function startPreparation() {
  ...
  const finish = (ok: boolean, prepared = 0, error = "备课未完成，请重试") => {
    ...
    state.status = { state: ok ? "ready" : "error", updatedAt: new Date().toISOString(), prepared, error: ok ? null : error };
  };
