...
test("consumes the pinned Core education_workspace snapshot from the separate data root", { skip: !coreRoot || !dataRoot }, async () => {
  ...
  assert.equal(result.workspace.students.length, 5);
  assert.equal(result.workspace.calendar.length, 28);
  assert.equal(result.workspace.tasks.length, 30);
  assert.deepEqual(result.payload.capabilities.supported_commands, ["review_observation", "review_memory_candidate", "review_teacher_context", "review_work_candidate", "review_task", "import_calendar", "import_timetable", "intake_material", "create_task", "move_task_stage"]);
