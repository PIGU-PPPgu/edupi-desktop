...
test("health requires the pinned Core memory-scope operation", async () => {
  const source = await readFile(new URL("./edupi-core-snapshot.ts", import.meta.url), "utf8");
  assert.match(source, /CORE_OPERATIONS = \["health", "snapshot", "command", "students", "delete", "kernel", "memory-scopes"\]/);
