...
export async function studentEventRequest(input:Record<string,unknown>):Promise<StudentEventsResult> {
  const {runtime,dataRoot}=resolveEduPiBridgeRoots();
  const requestId=crypto.randomUUID();
  const result=await runCoreProcess<StudentEventsResult & {operation:string;request_id:string;external_send:boolean}>({runtime,dataRoot,timeoutMs:15_000,request:{...input,protocol:"edupi-desktop-bridge",protocol_version:1,producer:"edupi-desktop",operation:"students",request_id:requestId}});
