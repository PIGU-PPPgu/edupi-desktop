...
import { normalizeDisplayMath, educationDocumentBody } from "./markdown.ts";

it("hides EduPi metadata only in the preview body", () => {
  const original = "---\ntask_id: task-1\ngenerated_by: EduPi\n---\n\n# 本节教案\n内容";
  assert.equal(educationDocumentBody(original), "# 本节教案\n内容");
  assert.match(original, /task_id/);
  assert.equal(educationDocumentBody("---\n普通标题\n---\n内容"), "---\n普通标题\n---\n内容");
  assert.equal(educationDocumentBody("---\ngenerated_by: EduPi\n未结束"), "---\ngenerated_by: EduPi\n未结束");
});
