...
test("every Desktop collaboration gets one stable context block without per-prompt duplication", async () => {
  let reads = 0;
  const dependencies = {
  ...
  assert.equal(typeof outsideEduPi, "function");
  assert.equal(outsideEduPi(["base instructions"]).join("\n").match(/<edupi_teacher_context>/g)?.length, 1);
