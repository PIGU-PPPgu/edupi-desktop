...
test("resolves and verifies the pinned Core runtime", { skip: !configuredRoot }, () => {
  ...
  const allowedRoot = process.env.EDUPI_CORE_ALLOWED_ROOT || path.dirname(realRoot);
  const manifestPath = path.join(realRoot, "contracts", "edupi-core-runtime-component-manifest.json");
  ...
  assert.equal(resolved.entrypoint, path.join(realRoot, "scripts", "core_runtime_daemon.mjs"));
  assert.equal(resolved.oneShotEntrypoint, path.join(realRoot, "scripts", "desktop_bridge_port.mjs"));
...
test("fails closed on runtime identity drift", { skip: !configuredRoot }, () => {
  const realRoot = fs.realpathSync(configuredRoot);
  const allowedRoot = process.env.EDUPI_CORE_ALLOWED_ROOT || path.dirname(realRoot);
  const manifest = JSON.parse(fs.readFileSync(path.join(realRoot, "contracts", "edupi-core-runtime-component-manifest.json"), "utf8"));
  assert.throws(() => resolveEduPiCoreRoot({ configuredRoot, allowedRoot, runtimeIdentity: { core_commit: "wrong", component_manifest_path: "contracts/edupi-core-runtime-component-manifest.json", component_manifest_hash: manifest.component_manifest_hash } }), /commit/i);
  assert.throws(() => resolveEduPiCoreRoot({ configuredRoot, allowedRoot, runtimeIdentity: { core_commit: execFileSync("git", ["-C", realRoot, "rev-parse", "HEAD"], { encoding: "utf8" }).trim(), component_manifest_path: "contracts/edupi-core-runtime-component-manifest.json", component_manifest_hash: "sha256:wrong" } }), /manifest hash/i);
});
