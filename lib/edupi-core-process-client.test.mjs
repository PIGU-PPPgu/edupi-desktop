...
test("calls real Core health, snapshot, and unsupported command", { skip: !configuredRoot || !configuredDataRoot }, async () => {
  ...
  assert.deepEqual(health.supported_commands, ["review_observation", "review_memory_candidate", "review_teacher_context", "review_work_candidate", "import_calendar", "import_timetable", "intake_material", "create_task", "move_task_stage"]);
  ...
  assert.equal(snapshot.envelope.payload.education_workspace.tasks.length, 30);
  ...
  const envelope = {
    ...fixture,
    message_id: "v11-review-task-unsupported-message",
    request_id: "v11-review-task-unsupported-request",
    idempotency_key: "v11-review-task-unsupported-idempotency",
    command: {
      ...reviewTaskBase,
  ...
  assert.equal(first.receipt.payload.reason_code, "unsupported_command");
});
