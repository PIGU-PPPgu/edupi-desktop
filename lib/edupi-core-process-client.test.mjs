...
test("daemon and one-shot modes both use the narrow broker without raw Core authority", async () => {
  for (const mode of ["daemon", "one-shot"]) {
    ...
    try {
      ...
      for (const operation of ["workspace-resources", "generated-artifacts"]) {
        await runCoreProcess({ runtime, dataRoot, timeoutMs: 5_000, request: {
          protocol: "edupi-desktop-bridge", protocol_version: 1, producer: "edupi-desktop",
          operation, request_id: `${mode}-${operation}`, action: "list",
        } });
        assert.equal(observed.at(-1).operation, operation === "workspace-resources" ? "bridge_read" : "bridge_call");
      }
    } finally {
      rendezvousModule.clearEduPiCoreRendezvousForTests();
      await broker.close();
