...
test("EduPi resource and native-tool gates recognize a symlink session cwd", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const helper = source.slice(source.indexOf("function normalizeRpcCwd("), source.indexOf("function getStartingSessionCwds("));
  const compiled = ts.transpileModule(helper, { compilerOptions: { target: ts.ScriptTarget.ES2022 } }).outputText;
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "edupi-rpc-cwd-"));
  try {
    const canonical = fs.realpathSync(root);
    const alias = path.join(root, "alias");
    fs.symlinkSync(canonical, alias, "dir");
    const gates = [...source.matchAll(/(\w+\((?:this\.cwd|sessionCwd)\)) === EDUPI_ROOT/g)];
    assert.equal(gates.length, 3);
    for (const [, expression] of gates) {
      const evaluate = new Function("resolve", "realpathSync", "sessionCwd", "EDUPI_ROOT", `${compiled}\nreturn ${expression} === EDUPI_ROOT;`);
      assert.equal(evaluate.call({ cwd: alias }, path.resolve, fs.realpathSync, alias, canonical), true);
      assert.equal(evaluate.call({ cwd: os.tmpdir() }, path.resolve, fs.realpathSync, os.tmpdir(), canonical), false);
    }
...
test("RPC session startup appends canonical EduPi context without replacing the Pi prompt", async () => {
  const source = await readFile(new URL("./rpc-manager.ts", import.meta.url), "utf8");
  const startupSource = source.slice(source.indexOf("export async function startRpcSession"));
  const contextIndex = startupSource.indexOf("createEduPiTeacherContextAppendSystemPromptOverride(");
  const servicesIndex = startupSource.indexOf("createAgentSessionServices(");

  assert.ok(contextIndex >= 0);
  assert.ok(servicesIndex > contextIndex);
  assert.match(startupSource, /appendSystemPromptOverride: teacherContextAppendSystemPromptOverride/);
  assert.doesNotMatch(startupSource, /systemPromptOverride: teacherContext/);
  assert.doesNotMatch(startupSource, /session\.send\([^)]*teacher.?context/is);
});
