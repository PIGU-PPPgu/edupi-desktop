...
test("finds the roster header after a title row", () => {
  assert.deepEqual(roster.parseStudentRosterCsv("七年级703班学生名单\n学号,姓名,性格特征\n001,蔡静然,认真\n"), [
    { name: "蔡静然", traits: ["认真"], parentNotes: [] },
  ]);
});
...
test("uses stable teacher-facing keys for student selection", () => {
  assert.equal(roster.studentRecordKey({ student_id: "student-1", name: "李四" }), "student-1");
  assert.equal(roster.studentRecordName({ display_name: "王五" }), "王五");
});

test("accepts namesakes only with distinct explicit classes", () => {
  assert.equal(roster.parseStudentRosterCsv("姓名,班级\n张三,703\n张三,704\n").length,2);
  assert.throws(()=>roster.parseStudentRosterCsv("姓名,班级\n张三,703\n张三,\n"),error=>error.code==="duplicate_name");
});
