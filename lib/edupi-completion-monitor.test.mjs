...
test("board and review changes refresh other views without repeating generation notifications", () => {
  const item = { ...task("one", "draft_ready"), status: "planned", revision: 0, boardStage: "progress", boardRevision: 0 };
  const baseline = monitor.completionSnapshot([item], "/workspace");
  for (const update of [{ boardStage: "done", boardRevision: 1 }, { status: "accepted", revision: 1 }]) {
    const changed = monitor.completionSnapshot([{ ...item, ...update }], "/workspace");
    assert.notEqual(monitor.completionSnapshotSignature(baseline), monitor.completionSnapshotSignature(changed));
    assert.deepEqual(monitor.diffTaskCompletionTransitions(baseline, changed), []);
    assert.equal(monitor.completionSnapshotSignature(changed), monitor.completionSnapshotSignature(monitor.completionSnapshot([{ ...item, ...update }], "/workspace")));
  }
});
