...
export function createStudentEventTool(projectRoot:string){
  return defineTool({
    ...
    execute:async(_call,params,_signal,_update,ctx)=>{
      if(resolve(ctx.cwd)!==resolve(projectRoot))throw new Error("请在 EduPi 工作区记录学生情况");
      if(params.action==="list"){
        const result=await studentEventRequest({action:"list_events",student:params.student,offset:params.offset});
        return {content:[{type:"text",text:JSON.stringify(result)}],details:result};
      }
      if(params.action==="update"){
        const result=await studentEventRequest({action:"update_event",event_id:params.event_id,expected_revision:params.expected_revision,summary:params.summary,topic:params.topic,observed_on:params.observed_on});
      ...
      const result=await studentEventRequest({action:"record_events",source:{session_id:ctx.sessionManager.getSessionId(),message_id:message.id,text},records:params.records});
